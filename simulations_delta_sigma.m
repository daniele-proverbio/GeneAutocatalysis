...
for n_n = 1 : length(n)                            
    for noise_m = 1 : length(noise)  
        for exp_n = 1 : N_Exp
            x_in = x_s(n_n) - delta + 2*delta*rand(1);             % Initial conditions (on upper branch) .  (initial conditions: x_in = [x_s + delta, x_s - delta])
            tic = 0;   
            exceed = 0;
...
for n_n = 1 : length(n)                            
    for de = 1 : length(deltas)  
        for exp_n = 1 : N_Exp
            x_in = x_s(n_n) - deltas(de) + 2*deltas(de)*rand(1);             % Initial conditions (on upper branch) .  (initial conditions: x_in = [x_s + delta, x_s - delta])
            tic = 0;   
            exceed = 0;
